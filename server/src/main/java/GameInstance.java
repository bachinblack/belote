...
public class GameInstance implements Runnable {
    ...
    private char[]   trump = {0x5, (char)0};
    private char     playerBet = 0;
    private char     cbIt = 0;
    private char[]   board = {32, 32, 32, 32};
    private short    Atotal = 0, Btotal = 0;
    boolean          gameOver = false;
    private char     id;

    GameInstance(Player player1, Player player2, Player player3, Player player4) {
        players[0] = player1;
        players[1] = player2;
        players[2] = player3;
        players[3] = player4;
    ...
    void setId(char lid) { id = lid; }

    public void run() {
        System.out.println("starting Game. id = " + (short)id);
        GameCommand cmd = new GameCommand();
        InitMsg msg = new InitMsg();
        int i;

        System.out.println("player 1: " + players[0].name);
        System.out.println("player 2: " + players[1].name);
        System.out.println("player 3: " + players[2].name);
        System.out.println("player 4: " + players[3].name);
        msg.threadId = id;
        ...
        for (i = 0; i < 4; ++i) {
            ...
            players[i].con.sendTCP(msg);
        }

        cmd.meta = 0;
        cmd.data = trump;
        players[0].con.sendTCP(cmd);
    ...
    void processResponse(char[] data) {
        GameCommand cmd = new GameCommand();
        char toDiv = (char)(cbIt / 4);

        switch (toDiv)
        {
            case 0: cmd.data = choseTrump(data); break;
            case 1: cmd.data = choseTrump(data); break;
            default: cmd.data = play(data[0]);
        }
        if (++cbIt == 40)
            FinalCount();
        else {
            ...
            players[cbIt % 4].con.sendTCP(cmd);
        }
    }
    ...
    boolean seekPlayer(Connection con) {
        for (Player p : players) {
            if (p.con == con) {
